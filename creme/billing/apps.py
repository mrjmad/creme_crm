...
class BillingConfig(CremeAppConfig):
    ...
    def ready(self):
        ...
        def check_deps(**kwargs):
            from .exporters import BillingExporterManager, BillingExporter

            errors = []
            ids = set()

            for exp_cls in BillingExporterManager().exporter_classes:
                if not issubclass(exp_cls, BillingExporter):
                    errors.append(
                        checks.Error(
                            f"the exporter {exp_cls} does not inherit <BillingExporter>.",
                ...
                if exp_cls.id in ids:
                    errors.append(
                        checks.Error(
                            f"the exporter {exp_cls} uses an id already used.",
                            hint='Check the BILLING_EXPORTERS setting in your'
                                 ' local_settings.py/project_settings.py',
                            obj=self.name,
